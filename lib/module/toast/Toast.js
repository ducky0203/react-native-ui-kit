...
export const Toast = ({
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    // Let TouchableOpacity handle taps
    onMoveShouldSetPanResponder: (_, gestureState) => {
      const canSwipe = isExpanded || stackIndex === 0;
      // Only capture if significant vertical movement
      return canSwipe && Math.abs(gestureState.dy) > 10 && Math.abs(gestureState.dy) > Math.abs(gestureState.dx);
    },
    ...
    onPanResponderRelease: (_, gestureState) => {
      if (!isExpanded && stackIndex !== 0) return;
      const threshold = 50;
      const shouldDismiss = position === 'top' && gestureState.dy < -threshold || position === 'bottom' && gestureState.dy > threshold;
      if (shouldDismiss) {
        hide();
      } else {
  ...
  useEffect(() => {
    ...
    Animated.parallel([Animated.spring(scale, {
      ...
      useNativeDriver: true,
      speed: 20,
      // Adjust speed to match approx 300ms feel
      bounciness: 0
  ...
  }, [stackIndex, targetScale, targetStackOffset, targetOpacity, isVisible, scale, stackTranslateY, opacity, isExpanded]);
